...
weighted_sorted_estimates <- read_rds(here("out", "weighted_sorted_estimates.rds"))
```
...
ordinary <- sorted_estimates%>%
   mutate(term=str_replace_all(term,"`",""))%>%
   ggplot(aes(estimate, fct_reorder(term, estimate)))+
   geom_col()+
   labs(x="Skill Income Elasticities",
        y="",
        subtitle="unweighted (OLS)")+
   theme_minimal()

weighted <- weighted_sorted_estimates%>%
   mutate(term=str_replace_all(term,"`",""))%>%
   ggplot(aes(estimate, fct_reorder(term, estimate)))+
   geom_col()+
   labs(x="Skill Income Elasticities",
        y="",
        subtitle="weighted by job openings (WLS)")+
   theme_minimal()

ordinary+weighted+plot_annotation("1% increase in skill ~ X% change in income")
```

Hedonic estimation paints a much different picture than aggregating the data. Consider the skill speaking.
When we aggregate the skill data, the skill of speaking is important for most occupations, resulting in a high aggregate score, regardless of how we weight the data when aggregating.  Hedonic estimation does something quite different.  Hedonic estimation finds that when the skill of speaking is abnormally high (compared with other occupations) income is abnormally low (compared with other occupations).  So while the skill of speaking is universally important, its relationship to income is negative. 
...
## Weighted Added Variable plots:

```{r}
weighted_sorted_estimates$av_plot[[1]]
```

```{r}
weighted_sorted_estimates$av_plot[[35]]
```
