...
skills <- skills_long%>%
  ...
  ungroup()|>
  mutate(across(where(is.numeric), ~ 100*rescale01(.x)), #put on same 0-100 scale
...
So we have two different measures of each skill: its level and importance, both on different scales.  The first step we take is to rescale the data so both level and importance are both on the same relative scale from 0--100, using function
...
The geometric mean has the attractive property that level and importance are (imperfect) complements when creating the composite measure. e.g. an importance of 40 and a level of 60 yields a lower composite measure than if both measures were 50: the difference between level and importance is penalized. Note that this example is extreme: the correlation between the level and importance is `r with(skill_dif, round(cor(Importance, Level),2))`, so the disparity between the level and importance is small.

```{r}
hist(skill_dif$abs_diff,
     main="Level and Importance are within +/- 10 for almost all values",
