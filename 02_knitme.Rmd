...
So we have two different measures of each skill: its level and importance, both on different scales.  The first step we take is to rescale the data so both level and importance are both on the same relative scale from 0--10, using function

$$f(x)=10\frac{x-\min(x)}{\max(x)-\min(x)}$$


Next, we use the rescaled variables to create a single measure that captures both the level and the importance. To do so we take the geometric mean of these two rescaled measures.

$$composite=\sqrt{level \times importance}$$

The geometric mean has the attractive property that level and importance are (imperfect) complements when creating the composite measure. e.g. an importance of 4 and a level of 6 yields a lower composite measure than if both measures were 5: the difference between level and importance is penalized. Note that this example is extreme: the correlation between the level and importance is `r with(skill_dif, round(cor(Importance, Level),2))`, so the disparity between the level and importance is small.
...
First thing we can do is compare the current and future average skills data. Note that because employment and job openings are so highly correlated the difference between the two weightings is minimal. Hovering over the points in the plot below the skills with the highest scores are `r tbbl%>%filter(thing=="composite")%>%slice_max(future, n=5)%>%pull(element_name)%>%paste(collapse = ", ")`.
